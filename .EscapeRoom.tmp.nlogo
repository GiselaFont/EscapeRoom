...
breed [youngs young]
turtles-own [energy vision-radius velocity]
...
to setup-turtles
  ...
  ask youngs [ set color blue ]
  ask averages [ set color orange ]
...
;;get the radius from the corresponding group of agents and check within that radius
to check-radius
  ;;check the agents within that radius
  ask youngs [
    let my-neighbors (other turtles) in-radius 3
    let num-neigbors count my-neighbors
    ;;show my-neighbors
    ask my-neighbors [ set color black ]
    let agent-with-max-velocity max-one-of turtles [ velocity ]
    let max-velocity [velocity] of agent-with-max-velocity
    set velocity max-velocity
    ;;ask agent-with-max-velocity [ velocity ]
  ]

  ;;ask youngs
  ;;[ ask patches in-radius young-vision-radius
    ;;[ set pcolor red ] ]
end
@#$#@#$#@
GRAPHICS-WINDOW
